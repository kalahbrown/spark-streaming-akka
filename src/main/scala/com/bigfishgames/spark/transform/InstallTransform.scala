...
//TODO: Impelment logging

class InstallTransform extends  Serializable with Transform {
  ...
  override def transformer(avroRdd: RDD[(AvroKey[GenericRecord], NullWritable)]): RDD[(AvroKey[GenericRecord], NullWritable)] = {
    avroRdd.map(row => parse(row))
  }
  
  override def inputSchemaStr = { inputSchema }
  override def outputSchemaStr = {outputSchema }
  ...
  private def osNameTransform(osInfo: String) = {
    if (osInfo == null) null
  ...
  private def hardwareManufacturerTransform(platform: Any) = {
